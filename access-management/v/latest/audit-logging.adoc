...
Anypoint MQ lets you log each action for creating and maintaining queues,
...
The following example lists a formatted version of the contents of `payload.txt`:
...
* An administrator has been alerted that an expected FIFO queue is no longer available. The administrator can check the Audit Logs and determine who deleted the queue and when. This lets the administrator understand the issue better.
* A sub-organization administrator has been alerted that a user has had their permissions removed for a given custom role. Using the Audit Log feature and examining the payload the administrator can determine what permissions were changed and by who.
* To search all actions of a user.
* To search for all sign-in attempts.
* To search all actions to an object and by whom.
...
Sample curl commands for accessing the API:
...
curl 'https://anypoint.mulesoft.com/audit/v2/organizations/<organization_id>/objectTypes?include_internal=false' -H 'Authorization: bearer <bearer_value>' 

curl 'https://anypoint.mulesoft.com/audit/v2/organizations/<organization_id>/actions?include_internal=false' -H 'Authorization: bearer <bearer_value>' 
...
curl 'https://anypoint.mulesoft.com/accounts/api/organizations/<organization_id>/users?id=<user_ID>' -H 'Authorization: bearer <bearer_value>'

curl 'https://anypoint.mulesoft.com/audit/v2/organizations/<organization_id>/objects?include_internal=false' -H 'Authorization: bearer <bearer_value>'
----
